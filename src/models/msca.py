...
from utils.utils import build_knn_graph_from_features
...
class MSCA(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.knn_build_batch_size = config['knn_build_batch_size'] or 1024
        ...
        if self.v_feat is not None:
            ...
            if os.path.exists(image_adj_file):
                ...
                image_adj = build_knn_graph_from_features(self.image_embedding.weight.detach(), self.knn_k, self.knn_build_batch_size, 'sym')
                torch.save(image_adj.cpu(), image_adj_file)
        ...
        if self.t_feat is not None:
            ...
            if os.path.exists(text_adj_file):
                ...
                text_adj = build_knn_graph_from_features(self.text_embedding.weight.detach(), self.knn_k, self.knn_build_batch_size, 'sym')
                torch.save(text_adj.cpu(), text_adj_file)
            self.text_original_adj = text_adj.to(self.device)
        if self.v_feat is not None:
            self.image_trs = nn.Linear(self.v_feat.shape[1], self.embedding_dim)
        if self.t_feat is not None:
            self.text_trs = nn.Linear(self.t_feat.shape[1], self.embedding_dim)
        self.query_common = nn.Sequential(nn.Linear(self.embedding_dim, self.embedding_dim), nn.Tanh(), nn.Linear(self.embedding_dim, 1, bias=False))
    ...
    def get_norm_adj_mat(self):
        ...
        norm_R = torch.sparse_coo_tensor(torch.as_tensor(np.array([norm_R.row, norm_R.col]), dtype=torch.long), torch.as_tensor(norm_R.data, dtype=torch.float32), norm_R.shape, device=self.device, check_invariants=False).coalesce()
        norm_adj_mat = torch.sparse_coo_tensor(torch.as_tensor(np.array([norm_adj_mat.row, norm_adj_mat.col]), dtype=torch.long), torch.as_tensor(norm_adj_mat.data, dtype=torch.float32), norm_adj_mat.shape, device=self.device, check_invariants=False).coalesce()
        return (norm_R, norm_adj_mat)

    def get_struct_adj_mat(self):
        inter_matrix = self.interaction_matrix.tocsr()
        item_cooccurrence = (inter_matrix.T @ inter_matrix).tocsr()
        item_cooccurrence.setdiag(0)
        item_cooccurrence.data[item_cooccurrence.data <= 1] = 0
        item_cooccurrence.eliminate_zeros()
        item_row, item_col = ([], [])
        for i in range(self.n_items):
            item_row.append(i)
            item_col.append(i)
            start = item_cooccurrence.indptr[i]
            end = item_cooccurrence.indptr[i + 1]
            columns = item_cooccurrence.indices[start:end]
            values = item_cooccurrence.data[start:end]
            k = min(len(columns), self.knn_k)
            if k > 0:
                selected = np.argpartition(values, -k)[-k:]
                item_row.extend([i] * k)
                item_col.extend(columns[selected].tolist())
    ...
    def compute_normalized_laplacian(self, indices, adj_size):
        adj = torch.sparse_coo_tensor(indices, torch.ones_like(indices[0], dtype=torch.float32), adj_size, device=indices.device, check_invariants=False).coalesce()
        row_sum = 1e-07 + torch.sparse.sum(adj, -1).to_dense()
        r_inv_sqrt = torch.pow(row_sum, -0.5)
        rows_inv_sqrt = r_inv_sqrt[indices[0]]
        cols_inv_sqrt = r_inv_sqrt[indices[1]]
        values = rows_inv_sqrt * cols_inv_sqrt
        return torch.sparse_coo_tensor(indices, values, adj_size, device=indices.device, check_invariants=False).coalesce()
