...
def build_knn_normalized_graph(adj, topk, is_sparse, norm_type):
    ...
    if is_sparse:
        ...
        return torch.sparse_coo_tensor(edge_index, edge_weight, adj.shape, device=device, check_invariants=False).coalesce()
    else:
        weighted_adjacency_matrix = torch.zeros_like(adj).scatter_(-1, knn_ind, knn_val)
        return get_dense_laplacian(weighted_adjacency_matrix, normalization=norm_type)

def build_knn_graph_from_features(context, topk, batch_size, norm_type):
    ...
    for start in range(0, total, batch_size):
        ...
        similarities = torch.matmul(context[start:end], context.transpose(0, 1))
        values, indices = torch.topk(similarities, topk, dim=1)
        rows = torch.arange(start, end, device=context.device).repeat_interleave(topk)
        row_parts.append(rows)
        col_parts.append(indices.reshape(-1))
        value_parts.append(values.reshape(-1))
    edge_index = torch.stack([torch.cat(row_parts), torch.cat(col_parts)])
    edge_weight = torch.cat(value_parts)
    edge_index, edge_weight = get_sparse_laplacian(edge_index, edge_weight, total, normalization=norm_type)
    return torch.sparse_coo_tensor(edge_index, edge_weight, (total, total), device=context.device, check_invariants=False).coalesce()
