...
class TopKEvaluator(object):
    ...
    def evaluate(self, batch_matrix_list, eval_data, is_test=False, idx=0):
        ...
        if self.save_recom_result and is_test:
            ...
            file_path = os.path.join(dir_name, '{}-{}-idx{}-top{}-{}.csv'.format(model_name, dataset_name, idx, max_k, get_local_time()))
    ...
    def __str__(self):
        mesg = 'The TopK Evaluator Info:\n' + '\tMetrics:[' + ', '.join([topk_metrics[metric.lower()] for metric in self.metrics]) + '], TopK:[' + ', '.join(map(str, self.topk)) + ']'
        return mesg
