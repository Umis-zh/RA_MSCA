...
class TrainDataLoader(AbstractDataLoader):
    ...
    def inter_matrix(self, form='coo', value_field=None):
        ...
        return self._create_sparse_matrix(self.dataset.df, self.dataset.uid_field, self.dataset.iid_field, form, value_field)
    ...
    def _get_neg_sample(self):
        ...
        if self.neighborhood_loss_required:
            ...
            batch_tensor = torch.cat((batch_tensor, neg_ids.unsqueeze(0), pos_neighbors.unsqueeze(0), neg_neighbors.unsqueeze(0)))
        else:
...
class EvalDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, additional_dataset=None, batch_size=1, shuffle=False):
        super().__init__(config, dataset, additional_dataset=additional_dataset, batch_size=batch_size, neg_sampling=False, shuffle=shuffle)
